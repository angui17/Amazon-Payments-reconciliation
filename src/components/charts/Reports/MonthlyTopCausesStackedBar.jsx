...
/**
 * Espera items como:
 * { month:"2025-01", amountAbs:17057.78, amountDescription:"Principal", ... }
 */
const MonthlyTopCausesStackedBar = ({ topCausesMonthly = [], loading }) => {
  const { labels, seriesKeys, matrixByKey } = useMemo(() => {
    ...
    for (const r of rows) {
      ...
      totalsByCause.set(cause, (totalsByCause.get(cause) || 0) + num(r.amountAbs ?? r.amount));
    }
    ...
    const matrix = Object.fromEntries(keys.map((k) => [k, Array(months.length).fill(0)]));
