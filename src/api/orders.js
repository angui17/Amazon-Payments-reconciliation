...
export async function getOrders(params = {}) {
  // Ignoring params (page/perPage) because external API is not paginated here.
    // Use the local dev proxy to avoid CORS issues: '/ida-proxy/...'
    ...
    // The IDA portal expects the token (and optionally ID/company) as query params
    // and the engine/fecha_desde/fecha_hasta (or last) in the JSON body.
    const body = { Token: token, token: token, engine: 'Worker' };
    ...
    if (!params || (!params.fecha_desde && !params.fecha_hasta)) body.last = params.last || 10;
    ...
    try {
      ...
      //     "engine": "Worker", 
      //     "name": "World",
      //     "fecha_desde": "03-14-2025",
      //     "fecha_hasta": "03-14-2025"
      //   }),
  ...
  headers: {
    //"accept": "*/*",
    "Token": "Gv54n6Et644L4GZ9VLluKX4GTOLfNiWuIST",
    "Id": 257,
    "Content-Type": "application/json"
  },
  body: JSON.stringify({
    ...
    if (!res.ok) {
      ...
      if (tokenError) {
        ...
        const altBody = Object.assign({ company: 'IDA', token: token, ID: 257 }, (params && params.fecha_desde) ? { fecha_desde: params.fecha_desde } : {}, (params && params.fecha_hasta) ? { fecha_hasta: params.fecha_hasta } : {}, (!params || (!params.fecha_desde && !params.fecha_hasta)) ? { last: params.last || 10 } : {});
        let altRes;
        try {
...
export async function getOrder(id) {
  // Fallback: fetch all and find by id (not efficient but acceptable client-side)
